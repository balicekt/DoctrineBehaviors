...
class EntitySortingService
{
	...
	public function moveBefore(string $scope, int $entityId, ?int $targetEntityId) : bool {
		...
		$entityPosition = $this->em->getRepository(EntityPosition::class)->findBy(['entityId' => $entityId]);
		if(!$entityPosition) {
		...
		if($targetEntityId) {
			...
			$targetEntityPosition = $this->em->getRepository(EntityPosition::class)->findBy(['entityId' => $targetEntityId]);
		...
		try {
			...
			if (!$targetEntityPosition) {
				// moving to the end
				$maxPos = $this->em->createQuery('SELECT max(ep.position) FROM ' . EntityPosition::class . ' ep WHERE scope = :scope')
	...
	public function checkPriorityPresence(string $scope, string $entityClass, $sortBy = ['id' => Criteria::ASC]) : void {
		if($this->em->getRepository($entityClass)->countBy([]) != $this->em->getRepository(EntityPosition::class)->countBy(['scope' => $scope])){
			...
			$q = $qb->getQuery();
			$position = $this->em->createQuery('SELECT max(ep.position) FROM ' . EntityPosition::class . ' ep WHERE scope = :scope')
				->setParameters(['scope' => $scope])->getSingleScalarResult();
	...
	public function applySorting(QueryBuilder $queryBuilder, string $entityAlias, string $direction = Criteria::ASC) : void {
		$queryBuilder->leftJoin(EntityPosition::class, 'ep', Join::WITH, 'ep.entityId = ' . $entityAlias)
